document.addEventListener('DOMContentLoaded', function() {
            ...
            function renderUserProfile(userId) {
                ...
                if (isMyProfile) {
                    ...
                    setTimeout(function() {
                        ...
                        // DEDUP FIX: clone-replace the button each time renderUserProfile runs
                        // so any previously bound click listeners are discarded before adding new one.
                        var profileSubmitBtn = document.getElementById('profile-post-submit-btn');
                        if (profileSubmitBtn) {
                            var _freshBtn = profileSubmitBtn.cloneNode(true);
                            profileSubmitBtn.parentNode.replaceChild(_freshBtn, profileSubmitBtn);
                            profileSubmitBtn = _freshBtn;
                            profileSubmitBtn.addEventListener('click', async function() {
                                ...
                                try {
                                    ...
                                    // Add to main dashboard feed (optimistic UI)
                                    var feedContainer = document.getElementById('feed-container');
                                    if (feedContainer) feedContainer.prepend(postEl);
                                    // NOTE: profile-dash-feed and profile-posts-feed are intentionally
                                    // NOT updated here. The Firestore onSnapshot listener handles
                                    // mirroring posts to those feeds with built-in duplicate guards.
                                    // Injecting here AND letting the listener run caused duplicate posts.
                                    // Save to Firestore
                ...
                // DEDUP FIX: profile-posts-feed is populated EXCLUSIVELY by the Firestore
                // onSnapshot listener (which has its own data-post-id dedup guard).
                // Cloning from #feed-container here caused duplicates every time
                // renderUserProfile() was called (on tab visit, after posting, etc.).
                // The empty-state div is managed by the listener; nothing to do here.
                ...
                // Only show empty state if feed truly has no posts yet
                if (profilePostsFeed && profilePostsEmpty) {
                    profilePostsEmpty.style.display = profilePostsFeed.children.length === 0 ? 'block' : 'none';
                }
            ...
            function navigateTo(targetId, fromClick = false) {
                ...
                try {
                    ...
                    // Show quick-post FAB only on dashboard, for all users
                    if (typeof updateQuickPostFab === 'function') updateQuickPostFab(targetId);
                        ...
                        case 'marketplace-form': {
                            ...
                            (async () => {
                                try {
                                    ...
                                    if (mktSubmitBtn) mktSubmitBtn.disabled = false;
                                    // Clear any modal-open state that might block navigation
                                    document.body.classList.remove('modal-open');
                                    document.body.style.overflow = '';
                                }
            ...
            (function() {
                ...
                (function buildMobileBottomNav() {
                    ...
                    const userItems = [
                        ...
                        { t: 'news',           icon: 'fa-newspaper',        label: 'News'     },
                        { t: 'marketplace',    icon: 'fa-store',            label: 'Market'   },
                        { t: 'report-crisis', icon: 'fa-exclamation-triangle', label: 'Crisis' },
